      SUBROUTINE PIMZRGMRESEV(X,B,WRK,IPAR,DPAR,MATVEC,PRECONL,PRECONR,
...
*     ..
*     .. Local Arrays ..
      DOUBLE COMPLEX DUMMY(1,1),EIGH(IBDIM),G(LDG),H(LDR,IBDIM),
     +               R(LDR,LDR),RHO(IBDIM),WRKEV(IBDIM)
*     ..
*     .. External Functions ..
...
*     .. External Subroutines ..
      EXTERNAL DECODE,ENCODE,GIVENS,PIMDGETPAR,ZAXPY,ZCOPY,ZHSEQR,ZINIT,
     +         ZSCAL,ZTRSV
*     ..
*     .. Intrinsic Functions ..
...
*  5. R(i,j)=dot(V(i),Q1AQ2V(j))
              K1 = 0
              DO 50 I = 1,J
                  H(I,J) = ZDOTC(LOCLEN,WRK(IV+K1),1,WRK(IZ),1)
                  R(I,J) = H(I,J)
                  K1 = K1 + LOCLEN
   50         CONTINUE
...
*  7. R(j+1,j)=||Vhat(j)||_2
              CALL PDZNRM2(H(J+1,J),LOCLEN,WRK(IW))
              R(J+1,J) = H(J+1,J)
...
*  15. compute eigenvalues of H, obtain bounding region
          CALL ZHSEQR('E','N',BASISDIM,1,BASISDIM,H,LDR,EIGH,DUMMY,1,
     +                WRKEV,IBDIM,INFO)
          DPAR(3) = DBLE(EIGH(1))
          DPAR(4) = DBLE(EIGH(1))
          DPAR(5) = DIMAG(EIGH(1))
          DPAR(6) = DIMAG(EIGH(1))
          DO 110 I = 2,BASISDIM
              DPAR(3) = MIN(DPAR(3),DBLE(EIGH(I)))
              DPAR(4) = MAX(DPAR(4),DBLE(EIGH(I)))
              DPAR(5) = MIN(DPAR(5),DIMAG(EIGH(I)))
              DPAR(6) = MAX(DPAR(6),DIMAG(EIGH(I)))
  110     CONTINUE
