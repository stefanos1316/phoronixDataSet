      SUBROUTINE PIMZTFQMR2(X,B,WRK,IPAR,DPAR,MATVEC,PRECONL,PRECONR,
     +                      PZSUM,PDZNRM2,PROGRESS,NNIT)
      IMPLICIT NONE
...
*
*     Modified by Jan Fostier (not the original PIM TFQMR!)
* ----------------------------------------------------------------------
*
*     .. Parameters ..
...
*     .. Local Scalars ..
     ...
     +        PRECONTYPE,PROCID,STATUS,STEPERR,STOPTYPE,NNIT,CNIT
*     ..
*     .. Local Arrays ..
...
* Modified: original : KAPPA.LT.EPSILON
              IF (KAPPA.EQ.EPSILON) THEN
...
*  Call monitoring routine
                  CALL PROGRESS(ITNO,ABS(KAPPA),EXITNORM)
...
*  Call monitoring routine
                  IF (CNIT.EQ.0) THEN
                        CALL STOPCRIT(B,WRK(IR),WRK(IZ),X,
     +                                WRK(IXOLD),WRK(IP),RHSSTOP,
     +                                CNVRTX,EXITNORM,STATUS,IPAR,
     +                                MATVEC,MATVEC,PRECONR,PZSUM,
     +                                PDZNRM2)
                        IF (STATUS.EQ.0) THEN
                                GO TO 30
                        END IF
